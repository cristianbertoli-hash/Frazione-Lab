(function(root, factory){
  ...
  function parseBinaryFrame(raw){
    ...
    return {transferId,transferIdHex:hex(transferId),index,frameCount,payloadLength,totalLength,hashPrefix,hashPrefixHex:hex(hashPrefix),crc32:storedCrc,payload,raw};
  }
  ...
  async function split(bytes, options={}){
    ...
    }else transferId=getCrypto().getRandomValues(new Uint8Array(8));
  ...
  return {VERSION,DEFAULT_PAYLOAD_BYTES,split,parseFrameText,reassemble,missingIndexes,encodeFrameText,decodeFrameText,parseFrameSetJson,crc32};
});
